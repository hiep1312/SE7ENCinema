                    ...
                    <div class="card-body">
                        <div class="d-flex justify-content-between">
                            <div>
                                ...
                                <h3 class="mb-0">{{ number_format($totalOrdersIn30Days * $movie->price, 0, '.', '.') }}đ</h3>
                                <small>VNĐ</small>
                            </div>
                            <div class="align-self-center">
                            ...
                            <div class="card-body bg-dark" style="border-radius: 0 0 var(--bs-card-inner-border-radius) var(--bs-card-inner-border-radius);">
                                <table class="table table-borderless text-light">
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-wrap text-muted lh-base text-start">{{ $movie->description ?? 'Không có mô tả' }}</td>
                                    </tr>
                                    <tr>
                                    ...
                                    <tr>
                                        ...
                                        <td><span style="color: #34c759;"><i class="fas fa-play me-1"></i>{{ $movie->release_date->format('d/m/Y') }}</span></td>
                                    </tr>
                                    <tr>
                                        <td><strong class="text-warning">Ngày kết thúc:</strong></td>
                                        <td><span style="color: #ff4d4f;"><i class="fas fa-stop me-1"></i>{{ $movie->end_date?->format('d/m/Y') ?? 'Vĩnh viễn' }}</span></td>
                                    </tr>
                                    <tr>
                                ...
                                <div class="video-container glow-effect mt-1">
                                    ...
                                    <div class="video-wrapper">
                                        <div class="responsive-iframe">
                                            <iframe src="{{ getYoutubeEmbedUrl($movie->trailer_url) ?: asset('storage/404.webp') }}"
                                        ...
                                        @foreach($recentShowtimes as $showtime)
                                            <div class="list-group-item bg-dark text-light border-warning">
                                                <div class="d-flex justify-content-between align-items-center">
                                                    ...
                                                    <span class="badge bg-{{ $showtime->status === 'canceled' ? 'danger' : 'success' }}">
                                        ...
                                        @foreach($ratings as $rating)
                                            <div class="list-group-item bg-dark text-light border-warning">
                                                <div class="d-flex justify-content-between align-items-center gap-3">
                                                    <div>
                                                        <h6 class="mb-2 fw-bold text-capitalize"><i class="fas fa-film me-2"></i>{{ $rating->user?->name ?? 'N/A' }}</h6>
                                    ...
                                    <div class="list-group">
                                        @foreach($comments as $comment)
                                            <div class="list-group-item bg-dark text-light border-warning">
                                                <div class="d-flex justify-content-between align-items-center gap-3">
                                                    <div>
                                                        <h6 class="mb-2 fw-bold text-capitalize"><i class="fas fa-film me-2"></i>{{ $comment->user?->name ?? 'N/A' }}</h6>
                                                        <p class="my-1" style="background: rgba(255, 255, 255, 0.05); border-radius: 8px; padding: 12px; border-left: 3px solid #ffc107; font-style: italic; line-height: 1.5; color: #e8e9ea;">
                                                    ...
                                                    <span class="badge bg-{{ $comment->status === 'active' ? 'success' : ($comment->status === 'hidden' ? 'primary' : 'danger') }} d-flex gap-1 justify-content-center align-items-center">
                                                        {{ $comment->status === 'active' ? 'Đã đăng' : ($comment->status === 'hidden' ? 'Đã ẩn' : 'Đã xóa') }}
                                                    </span>
                                                </div>
                                            </div>
                                        @endforeach
