            ...
            <div class="row align-items-center g-1">
                <div class="col-sm-8">
                    ...
                    <div class="d-flex align-items-center">
                        <img src="{{ asset('storage/' . ($movie->poster ?? '404.webp')) }}" alt="Poster phim"
                            class="booking-food-movie-poster me-3">
                        <div>
                            <h5 class="mb-1">{{ $movie->title }}</h5>
                            <p class="mb-0" style="color: #777;">
                                <i class="fas fa-calendar-alt me-2"></i>{{
                                mb_ucfirst($booking->showtime->start_time->translatedFormat('l, d/m/Y - H:i'), 'UTF-8')
                                }}
                            </p>
                            <p class="mb-0" style="color: #777;">
                                <i class="fas fa-door-open me-2" style="width: 11px;"></i>Phòng chiếu: {{
                                $booking->showtime->room->name }}
                            </p>
                            <p class="mb-0" style="color: #777;">
                                <i class="fas fa-map-marker-alt me-2"></i>Rạp SE7ENCinema - Ghế: {{
                                $booking->seats->implode('seat_label', ', ') }}
                            </p>
                        </div>
                    </div>
                </div>
                    ...
                    {{-- <div class="row">
                        <div class="col-md-6 mb-4">
                            <div class="booking-food-item-card">
                                <img src="/placeholder.svg?height=200&width=300" alt="Combo 1"
                                    class="booking-food-item-image">
                            ...
                            <div class="booking-food-item-card" data-food-id="{{ $foodItem->id }}" wire:ignore.self>
                                <div class="booking-food-card-inner">
                                    <div class="booking-food-card-front">
                                        <img src="{{ asset(" storage/" . ($foodItem->image ?? '404.webp')) }}" alt="Ảnh
                                        đồ ăn & thức uống {{ $foodItem->name }}" class="booking-food-item-image">
                                        <div class="booking-food-item-content">
                                            <h5 class="booking-food-item-name">{{ $foodItem->name }}</h5>
                                            <p class="booking-food-item-desc">{{ $foodItem->description }}</p>

                                            @foreach($foodItem->availableAttributes as $foodAttribute =>
                                            $foodAttributeValues)
                                            <div class="booking-food-variant-group"
                                                data-attribute="{{ $foodAttribute }}" wire:key="{{ " $foodItem->id -
                                                $foodAttribute" }}" wire:ignore.self>
                                                <label class="booking-food-variant-label text-start">{{ $foodAttribute
                                                    }}:</label>
                                                <div class="booking-food-variant-options">
                                                    @foreach ($foodAttributeValues as $foodAttributeValue)
                                                    <div class="booking-food-variant"
                                                        data-value="{{ $foodAttributeValue }}"
                                                        onclick="selectVariant(event)" wire:key="{{ " $foodItem->id -
                                                        $foodAttribute - $foodAttributeValue" }}"
                                            ...
                                            <div class="booking-food-base-price">
                                                Giá tiền: <span class="booking-food-price-value" wire:ignore>{{
                                                    number_format($foodItem->variantsData->min('price'), 0, '.', '.')
                                                    }}đ - {{ number_format($foodItem->variantsData->max('price'), 0,
                                                    '.', '.') }}đ</span>
                                            </div>

                                            <button class="booking-food-auto-flip-btn"
                                                onclick="toggleTempCartItem(this, true)" wire:ignore>
                                        ...
                                        <div class="booking-food-item-content">
                                            ...
                                            <div class="booking-food-back-variants">
                                                ...
                                                <div class="booking-food-variants-summary">
                                                    @foreach($cartTempVariant->variantAttributes as $attribute =>
                                                    $value)
                                            ...
                                            <div class="booking-food-back-price-info">
                                                <div class="booking-food-back-unit-price">
                                                    Giá gốc: <span class="booking-food-back-original-price">{{
                                                        number_format($cartTempVariant->price, 0, '.', '.') }}đ</span>
                                                </div>
                                                <div class="booking-food-back-total-price">
                                                    Tổng tiền: <span class="booking-food-back-final-price"
                                                        x-text="(($wire.cartTempVariantQuantity || 1) * @json($cartTempVariant->price)).toLocaleString('vi').concat('đ')"></span>
                                                </div>
                                            </div>

                                            @php $availableQuantity = array_key_exists($cartTempVariantId, $carts) ?
                                            $carts[$cartTempVariantId][0] : 0; @endphp
                                            <div class="booking-food-quantity-section">
                                                ...
                                                <div class="booking-food-back-quantity-controls">
                                                    <button class="booking-food-back-btn-minus"
                                                        @click="$wire.$set('cartTempVariantQuantity', $wire.cartTempVariantQuantity <= 1 ? 1 : --$wire.cartTempVariantQuantity)"
                                                        :disabled="$wire.cartTempVariantQuantity <= 1">
                                                        <i class="fas fa-minus"></i>
                                                    </button>
                                                    <span class="booking-food-back-quantity"
                                                        x-text="$wire.cartTempVariantQuantity"></span>
                                                    <button class="booking-food-back-btn-plus"
                                                        @click="$wire.$set('cartTempVariantQuantity', $wire.cartTempVariantQuantity >= @json($cartTempVariant->quantity_available) ? $wire.cartTempVariantQuantity : ++$wire.cartTempVariantQuantity)"
                                                        :disabled="$wire.cartTempVariantQuantity >= @json($cartTempVariant->quantity_available - $availableQuantity)">
                                                        <i class="fas fa-plus"></i>
                                                    </button>
                                                </div>
                                            </div>
                                            ...
                                            <div class="booking-food-back-actions">
                                                <button class="booking-food-btn-back-to-variants"
                                                    onclick="toggleTempCartItem(this, false)">
                                                    <i class="fas fa-arrow-left"></i>
                                                    Quay lại
                                                </button>
                                                <button class="booking-food-btn-add-to-cart" @click="$wire.addToCart()"
                                                    wire:loading.attr="disabled" wire:target="addToCart"
                                                    :disabled="$wire.cartTempVariantQuantity > @json($cartTempVariant->quantity_available - $availableQuantity) || $wire.cartTempVariantQuantity === 0">
                ...
                <div class="booking-food-summary-card">
                    ...
                    <div class="booking-food-summary-section" wire:ignore>
                        ...
                        <div class="booking-food-summary-item small">
                            <span>{{ mb_ucfirst($booking->showtime->start_time->translatedFormat('l, d/m/Y - H:i'),
                                'UTF-8') }}</span>
                        </div>
                        <div class="booking-food-summary-item small">
                        ...
                        <div class="booking-food-seat-details">
                            ...
                            <div class="booking-food-summary-item">
                                ...
                                <span>{{ number_format($seats->get('standard')->first()->price +
                                    ($booking->showtime->movie?->price ?? 0), 0, '.', '.') }}đ</span>
                            </div>
                            @endif
                            @if($seats->has('vip'))
                            <div class="booking-food-summary-item">
                                ...
                                <span>{{ number_format($seats->get('vip')->first()->price +
                                    ($booking->showtime->movie?->price ?? 0), 0, '.', '.') }}đ</span>
                            </div>
                            @endif
                            @if($seats->has('couple'))
                            <div class="booking-food-summary-item">
                                ...
                                <span>{{ number_format($seats->get('couple')->first()->price +
                                    ($booking->showtime->movie?->price ?? 0), 0, '.', '.') }}đ</span>
                            </div>
                            @endif
                            <div class="booking-food-summary-item fw-bold border-top pt-2" style="margin-bottom: 0;">
                                <span>Tổng tiền vé:</span>
                                <span class="text-primary">{{ number_format($booking->bookingSeats->sum('ticket_price'),
                                    0, '.', '.') }}đ</span>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="booking-food-summary-items">
                            ...
                            <div class="booking-food-summary-item" wire:key="cart-variant-{{ $variantId }}">
                                <div class="flex-grow-1">
                                    ...
                                    <div class="text-muted small">{{ $variant->variantAttributes?->map(fn($value,
                                        $attribute) => "$attribute: $value")->implode(', ') }}</div>
                                    <div class="booking-food-summary-item-controls">
                                        <div class="booking-food-quantity-control">
                                            <button class="booking-food-summary-btn-minus"
                                                @click="$wire.carts[@json($variantId)]?.[0] && $wire.carts[@json($variantId)][0]--"
                                                :disabled="$wire.carts[@json($variantId)]?.[0] <= 1">
                                                <i class="fas fa-minus"></i>
                                            </button>
                                            <span class="booking-food-summary-quantity"
                                                x-text="$wire.carts[@json($variantId)]?.[0]"></span>
                                            <button class="booking-food-summary-btn-plus"
                                                @click="$wire.carts[@json($variantId)]?.[0] && $wire.carts[@json($variantId)][0]++"
                                                :disabled="$wire.carts[@json($variantId)]?.[0] >= @json($variant->quantity_available)">
                                                <i class="fas fa-plus"></i>
                                            </button>
                                        </div>
                                        <button class="booking-food-remove-btn"
                                            @click="delete $wire.carts[@json($variantId)]; Object.keys($wire.carts).length === 0 && $wire.$refresh()"
                                            onclick="this.closest('.booking-food-summary-item').style.display = 'none'">
                                ...
                                <div class="text-end">
                                    <span class="fw-bold"
                                        x-text="($wire.carts[@json($variantId)]?.[0] * @json($variant->price)).toLocaleString('vi').concat('đ')"></span>
                                    <div class="text-muted small">{{ number_format($variant->price, 0, '.', '.') }}đ x
                                        <span x-text="$wire.carts[@json($variantId)]?.[0]"></span></div>
                                </div>
                            </div>
                            @endforeach
                            @else
                    ...
                    <div class="booking-food-summary-total" wire:ignore>
                        <div class="d-flex justify-content-between fw-bold fs-5">
                            ...
                            <span id="total" class="text-danger"
                                x-text="(Object.values($wire.carts).reduce((total, [quantity, price]) => total + (quantity * (price ?? 0)), 0) + @json($booking->bookingSeats->sum('ticket_price'))).toLocaleString('vi').concat('đ')"></span>
                        </div>
                    </div>

                    <!-- Action Buttons -->
                    <div class="booking-food-summary-actions">
                        <a class="booking-food-btn-back"
                            href="{{ route('client.booking.select_seats', $booking->showtime_id) }}">
