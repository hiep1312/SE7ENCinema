...
@endassets
<div class="scRender scPromotion" wire:poll.6s>
                                ...
                                <div class="voucher-main-modern d-flex align-items-center mb-2 mt-3">
                                    ...
                                    <div style="min-width:0; margin: 10px 0px !important">
                                        <span class="fw-bold text-gradient-orange voucher-discount"
                                            style="font-size: 1.7rem;">Giảm
                                            {{ $promotion->discount_type === 'percentage' ? $promotion->discount_value . '%' : number_format($promotion->discount_value, 0, '.', '.') . 'đ' }}</span>
                                    </div>
                                </div>
                                <div class="mb-1" style="gap: 0.5rem; ">
                                    <span
                                        class="badge bg-gradient-orange text-white px-3 py-1 fs-6 fw-bold shadow-sm voucher-code-modern">{{ $promotion->code }}</span>
                                </div>
                                <div class="my-1 text-secondary small">Tiêu đề: <span
                                        class="fw-medium text-dark">{{ $promotion->title }}</span></div>
                                <span class="text-secondary small" style="font-size: 0.95rem;">Đơn tối thiểu: <b
                                        class="text-dark">{{ number_format($promotion->min_purchase, 0, '.', '.') }}đ</b></span>
                                <div class="text-secondary small" style="margin:5px 0px !important ">HSD: <b
                                        class="text-dark">{{ $promotion->end_date->format('Y/m/d H:i') }}</b></div>
                                <div class="text-secondary small">Số lượng còn lại: <span
                                        class="fw-medium text-dark">{{ $promotion->usage_limit - $promotion->used_count }}</span>
                                </div>
...
{{-- Lưu ý: Chưa có hàm saveVoucher nếu làm FOOOD thì tạo hàm hoặc route nhảy sang --}}

                                            <button {{-- wire:click="saveVoucher({{ $promotion->id }})" --}}
                                                class="btn btn-warning color1 fw-bold px-3 py-1 rounded-pill shadow-sm border-0">
                                                Lưu voucher
                                            </button>
                                        @endif
                                ...
                                <div class="collapse animate__animated animate__fadeIn"
                                    id="collapsePromoModern{{ $promotion->id }}" wire:ignore.self>
                        ...
                        @empty
                            <div class="alert alert-info">Bạn đã sử dụng hết các mã khuyến mãi hiện có hoặc chưa có khuyến
                                mãi nào khả dụng.</div>
                        @endforelse
                    </div>
                </div>
                            ...
                            <div class="col-lg-6 col-md-6 col-sm-12">
                                <div class="card movie-hot p-0 border-0" style="height: 440px; min-height: 440px;">
                                    <div class="position-relative movie-img-wrapper"
                                        ...
                                        <div class="ratio ratio-4x5 rounded-4 overflow-hidden">
                                            <img src="{{ $movie->poster ? asset('storage/' . $movie->poster) : asset('404.webp') }}"
                                                 class="card-img-top movie-img-large object-fit-cover"
                                                 alt="{{ $movie->title }}">
                                        </div>                                        
                                    ...
                                    <div class="card-body text-center p-2">
                                        <h5 class="card-title mb-2 movie-title-main"
                                            style="max-width: 100%; font-size: 1.15rem; font-weight: 700; color: #1976d2;">
                                        ...
                                        <div class="modal-content"
                                            ...
                                            <div class="modal-body p-0" style="position:relative;">
                                                <div class="trailer-bg-blur"
                                                    style="position:absolute;top:0;left:0;width:100%;height:100%;background:url('{{ $movie->poster ? asset('storage/' . $movie->poster) : asset('404.webp') }}') center center / cover no-repeat;filter: blur(16px) brightness(0.5);z-index:1;">
                                                </div>
                                                <div class="video-container glow-effect mt-1"
                                                    ...
                                                    <div class="video-wrapper">
                                                        <div class="responsive-iframe" style="border-radius: 1rem; overflow: hidden; background: #222;">
                                                            ...
                                                            @else
                                                                <img src="{{ asset('storage/404.webp') }}"
