...
<div class="container-fluid d-flex justify-content-center align-items-start scPromotion">
    <div class="container">
        ...
        <div class="promotion-tips-box mt-4 mb-4">
            <div class="promotion-tips-title"><i class="fa fa-lightbulb-o me-2"></i> Mẹo & Thông tin khuyến mãi</div>
            <div class="promotion-tip"><i class="fa fa-check-circle"></i> <strong>TIP 1: </strong> Theo dõi trang này để cập nhật các mã giảm giá, voucher và chương trình giảm giá mới nhất từ SE7ENCinema.</div>
            <div class="promotion-tip"><i class="fa fa-info-circle"></i> <strong>TIP 2: </strong> Mỗi voucher đều có điều kiện áp dụng riêng, hãy đọc kỹ thông tin trước khi sử dụng để nhận ưu đãi tốt nhất!</div>
            <div class="promotion-tip"><i class="fa fa-gift"></i> <strong>TIP 3: </strong> Số lượng voucher có hạn, nhanh tay lưu và sử dụng trước khi hết hạn nhé!</div>
            <div class="alert alert-warning mt-3 mb-0 py-2 px-3"><i class="fa fa-exclamation-triangle me-2"></i> Nếu gặp vấn đề khi sử dụng mã, vui lòng liên hệ bộ phận <a href="#">CSKH</a> để được hỗ trợ nhanh nhất.</div>
        </div>
        <div class="row promotion-main-row" style="--bs-gutter-x: 1.2rem !important;">
            <!-- Left: Promotions List -->
            <div class="col-lg-6 col-md-12 mb-4">
                <h2 class="fw-bold mb-4" style="font-family: 'Oswald', sans-serif; letter-spacing: 1px; display: inline-block; margin-bottom: -30px !important;" >MÃ GIẢM GIÁ</h2>

            <div class="respon">
                @forelse($promotions as $promotion)
                    <div class="voucher-card-modern my-4 position-relative animate__animated animate__fadeInUp">
                        <!-- Ribbon nhỏ hơn -->
                        <div class="voucher-ribbon position-absolute top-0 start-0 px-3 py-1 fw-bold text-white" style="font-size: 1rem; border-bottom-right-radius: 1rem;">
                        ...
                        <div class="voucher-main-modern d-flex align-items-center mb-2 mt-3">
                            <div class="voucher-icon rounded-circle d-flex align-items-center justify-content-center me-2" style="width: 40px; height: 40px; font-size: 1.2rem;">
                        ...
                        <div class="d-flex justify-content-between align-items-center mt-2" style="gap: 0.5rem;">
                            <div>
                                <button
                                    ...
                                    data-bs-target="#collapsePromoModern{{ $promotion->id }}"
                                    aria-expanded="false"
            ...
            <div class="col-lg-6 col-md-12" style="text-align: center !important">
                <h2 class="fw-bold mb-4 " style="font-family: 'Oswald', sans-serif; letter-spacing: 1px; display: inline-block; ">PHIM ĐANG HOT</h2>
                <div class="row g-4">
                        ...
                        <div class="col-lg-6 col-md-6 col-sm-12">
                            <div class="card movie-hot p-0 border-0" style="height: 440px; min-height: 440px;">
                                <div class="position-relative movie-img-wrapper" style="height: 400px; border-radius: 1.2rem; overflow: hidden;">
                                    ...
                                    @endif
                                    <div class="movie-overlay" style="position:absolute;top:0;left:0;width:100%;height:100%;display:flex;align-items:center;justify-content:center;">
                                        <div class="play-btn" data-bs-toggle="modal" data-bs-target="#movieTrailerModal{{ $movie->id }}" style="width: 60px; height: 60px; background: rgba(0,0,0,0.45); border-radius: 50%; display: flex; align-items: center; justify-content: center; cursor: pointer;">
                                            <i class="fa fa-play" style="color: #fff; font-size: 2rem;"></i>
                                        </div>
                                    </div>
                                </div>
                                <div class="card-body text-center p-2">
                                    <a href="{{route('client.movie_detail',$movie->id)}}" class="card-title mb-2 movie-title-main text-decoration-none" style="max-width: 100%; font-size: 25px; font-weight: 700; color: #1976d2;">{{ Str::limit($movie->title, 40) }}</a>
                                </div>
                            </div>
                            <!-- Modal trailer riêng cho từng phim, style giống admin, có background poster mờ, nếu không có trailer thì hiện 404.webp -->
                            <div class="modal modal-backdrop fade" id="movieTrailerModal{{ $movie->id }}" wire:ignore tabindex="-1" aria-labelledby="movieTrailerModalLabel{{ $movie->id }}" aria-hidden="true">
                                <div class="modal-dialog modal-lg modal-dialog-centered">
                                    <div class="modal-content" style="background: none; box-shadow: none; border: none;">
                                        <div class="modal-body p-0" style="position:relative;">
                                            <div class="trailer-bg-blur" style="position:absolute;top:0;left:0;width:100%;height:100%;background:url('{{ Str::startsWith($movie->poster ?? '', ['http://', 'https://']) ? $movie->poster : asset('storage/' . ($movie->poster ?? '404.webp')) }}') center center / cover no-repeat;filter: blur(16px) brightness(0.5);z-index:1;"></div>
                                            <div class="video-container glow-effect mt-1" style="position:relative;z-index:2;">
                                                ...
                                                <div class="video-wrapper">
                                                    <div class="responsive-iframe" style="border-radius: 1rem; overflow: hidden; background: #222;">
                                                        @if($movie->trailer_url)
                                                            <iframe data-src="{{ getYoutubeEmbedUrl((string)$movie->trailer_url) }}" allowfullscreen style="width:100%;"></iframe>
                                                        @else
                                                            <img src="{{ asset( 'storage/404.webp') }}" alt="404 Not Found" style="width:100%;height:360px;object-fit:contain;background:#fff;">
                                                        @endif
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="modal-footer justify-content-center" style="z-index:3;position:relative;">
                    ...
                    @empty
                        <div class="col-12"><div class="alert alert-info">Hiện chưa có phim hot nào.</div></div>
                    @endforelse
                </div>
            </div>
        </div>
