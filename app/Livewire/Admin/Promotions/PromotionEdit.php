...
class PromotionEdit extends Component
{
    ...
    protected $messages = [
        ...
        'usage_limit.min' => 'Giới hạn sử dụng phải lớn hơn hoặc bằng số lượt đã sử dụng hoặc 1 nếu chưa có giới hạn.',
    ...
    public function mount(Promotion $promotion)
    {
        ...
        $this->fill($promotion->only('title', 'description', 'discount_type', 'discount_value', 'code', 'usage_limit', 'min_purchase', 'status') + ['start_date' => $promotion->start_date->format('Y-m-d\TH:i'), 'end_date' => $promotion->end_date->format('Y-m-d\TH:i')]);
    }
   ...
   public function updatePromotion()
    {
        ...
        if($this->start_date !== $this->promotion->start_date->format('Y-m-d\TH:i')) $this->rules['start_date'] .= "|after_or_equal:now";
        if($this->end_date !== $this->promotion->end_date->format('Y-m-d\TH:i')) $this->rules['end_date'] .= "|after_or_equal:now";
        if($this->promotion->usages) $this->rules['usage_limit'] .= "|min:" . $this->promotion->usages->count();
        ...
        $this->promotion->update([
            ...
            'status' => strtotime($this->end_date) < time() ? "expired" : $this->status,
        ]);
    ...
    public function render()
    {
        ...
        $usages = $this->promotion->usages()->with('booking.user')->paginate(10);
        return view('livewire.admin.promotions.promotion-edit', compact('usages'));
    }
}
